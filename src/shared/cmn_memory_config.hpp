...
namespace Ripple
{
  ...
  namespace Physical
  {
    ...
    static_assert( DFLT_STATIC_PAYLOAD_SIZE <= 32 );    // Hardware limit
  }                                                     // namespace Physical
  ...
  {
    /**
     *  Set an upper limit on the number of bytes the transport layer can
    ...
    /**
     *  This will set the maximum number of datagrams that can be stored
     *  at any given time at the transport layer. This pool is shared amongst
     *  all endpoints for the purpose of fragmented packet deconstruction and
     *  reconstruction. See the transport layer Service class for more details
     *  on how that all works.
     *
     *  The memory allocated in bytes is equal to:
     *    DFLT_STATIC_PAYLOAD_SIZE * DATAGRAM_MESSAGE_POOL_SIZE
     */
    static constexpr size_t DATAGRAM_MESSAGE_POOL_SIZE = 25;
    ...
    namespace _Internal
    {
      static constexpr size_t _alloc_size = DATAGRAM_MESSAGE_POOL_SIZE * Physical::DFLT_STATIC_PAYLOAD_SIZE;
  ...
  namespace _Internal
  {
    ...
    static constexpr size_t TransportAlloc = Transport::_Internal::_alloc_size;

    static constexpr size_t AllLayerAlloc = DataLinkAlloc + TransportAlloc;
